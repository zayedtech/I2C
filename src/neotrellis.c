...
bool neopixel_begin(uint8_t internal_pin) {
    ...
    } else { printf("BUF Length set successfully\n");}
    sleep_ms(20);
...
bool neopixel_set_one_and_show(int idx, uint8_t r, uint8_t g, uint8_t b) {
    ...
    uint16_t start_addr = (uint16_t)idx * 3;

    uint8_t payload[5];
    

    payload[0] = start_addr & 0xFF;        
    payload[1] = (start_addr >> 8) & 0xFF; 
    ...
    if (!seesaw_write(NEOTRELLIS_ADDR, SEESAW_NEOPIXEL_BASE, NEOPIXEL_BUF, payload, sizeof(payload))) {
        printf("[ERR] BUF write failed (Address Endianness Issue?)\n");
        return false;
    }
